    ...
    def load_image(self, image_id, mode="RGBD"):
        ...
        if mode == "RGBD":
            ...
            if PROCESS_DEPTH == "inpaint":
                ...
                opening = (1 - cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=5)) * mask
                depth = cv2.inpaint(depth_raw.astype(np.uint8), opening.astype(np.uint8), 3, cv2.INPAINT_NS)
            else:
                depth = depth_raw
