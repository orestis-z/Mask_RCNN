...
class Config(ObjectsConfig):
    ...
    MEAN_PIXEL = np.array([123.7, 116.8, 103.9, 255.0 / 2]
                          ) if RGBD else np.array([123.7, 116.8, 103.9])
...
class Dataset(ObjectsDataset):
    ...
    # dump dataset config into pickle file since loading all infos takes long
    # time
            ...
            if root_split[-1] == 'photo':  # and subset in root_split:
                ...
                for file in files:
                    ...
                    depth_path = os.path.join(
                        parent_path, 'depth', file_name + '.png')
                    mask_path = os.path.join(
                        parent_path, 'instance', file_name + '.png')
    ...
    def load_mask(self, image_id):
        ...
        if 0 in instances:
            instances.remove(0)
        instances = [
            x for x in instances if instance_to_class[x] not in EXCLUDE]
        n_instances = len(instances)
        masks = np.repeat(np.expand_dims(mask_img, axis=2),
                          n_instances, axis=2)  # bottleneck code
        ...
        if BINARY_CLASS:
            ...
            class_ids = np.array(
                [instance_to_class[x] + 1 for x in instances], dtype=np.uint32)
...
if __name__ == '__main__':
    dataset = Dataset()
    dataset.save_dataset_config(
        '/external_datasets/SceneNet_RGBD', 'validation')
