...
if RGBD:
    SCENENET_MODEL_PATH = os.path.join(
        MODEL_DIR, 'scenenet20180428T1942/mask_rcnn_scenenet_0500.h5')
else:
    SCENENET_MODEL_PATH = os.path.join(
        MODEL_DIR, 'scenenet_coco_rgb20180428T1942/mask_rcnn_scenenet_coco_rgb_0596.h5')
...
if init_with == 'last':
    ...
    config_rgbd.IMAGE_SHAPE = np.array(
        [config.IMAGE_MAX_DIM, config.IMAGE_MAX_DIM, 4])
    config_rgbd.display()

    model_rgbd = modellib.MaskRCNN(
        mode='training',
        config=config_rgbd,
        model_dir=MODEL_DIR)
    ...
    kernel_rgbd = np.concatenate(
        (kernel_rgb,
         np.mean(kernel_rgb, keepdims=True, axis=2)),
        axis=2)
