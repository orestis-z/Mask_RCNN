"""Mask R-CNN The main Mask R-CNN model implemenetation.
...
def log(text, array=None):
    ...
    And, optionally, if a Numpy array is provided it prints it's shape,
    min, and max values.
    """
...
class BatchNorm(KL.BatchNormalization):
    """Batch Normalization class. Subclasses the Keras BN class and hardcodes
    training=False so the BN layer doesn't update during training.

    Batch normalization has a negative effect on training if batches are
    small so we disable it here.
    """
...
def log2_graph(x):
    """Implementatin of Log2.

    TF doesn't have a native implemenation.
    """
...
def build_fpn_mask_graph(rois, feature_maps,
                         image_shape, pool_size, num_classes):
    """Builds the computation graph of the mask head of Feature Pyramid
    Network.
...
def load_image_gt(dataset, config, image_id, augment=False,
                  use_mini_mask=False):
    """Load and return ground truth data for an image (image, mask, bounding
    boxes).
    ...
    if augment:
        ...
        if random.randint(0, 1):
            img = skimage.util.random_noise(
                img / 255, var=random.uniform(0, 0.005)) * 255
        if config.MODE == 'RGB':
            image = img
        elif config.MODE == 'RGBD':
            depth = image[:, :, 3]
            if random.randint(
                    0, 1):  # add noise to depth independend of noise on rgb
                depth = skimage.util.random_noise(
                    depth / 255, var=random.uniform(0, 0.005)) * 255
        ...
        if random.randint(0, 1):
            ...
            assert image.shape == image_shape, "Augmentation shouldn't change image size {} -> {}".format(
                image_shape, image.shape)
            assert mask.shape == mask_shape, "Augmentation shouldn't change mask size {} -> {}".format(
                mask_shape, mask.shape)
        # blur
        img = image[:, :, 0:3]
        if random.randint(0, 1):
...
def build_detection_targets(
    ...
    not used in normal training. It's useful for debugging or to train the Mask
    RCNN heads without using the RPN head.
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == 'training':
            ...
            _, _, _, active_class_ids = KL.Lambda(
                lambda x: parse_image_meta_graph(x), mask=[
    ...
    def find_all(self):
        ...
        # Pick directories
        dir_names = [os.path.join(self.model_dir, dir_name)
        ...
        for dir_name in dir_names:
            ...
            checkpoints = sorted(checkpoints)
            checkpoints = [os.path.join(dir_name, checkpoint)
    ...
    def load_weights(
            ...
            exclude=None,
            include=None):
        """Modified version of the correspoding Keras function with the
        addition of multi-GPU support and the ability to exclude some layers
        from loading.
    ...
    def compile(self, learning_rate, momentum):
        ...
        Adds losses, regularization, and metrics. Then calls the Keras
        compile() function.
        """
        # Optimizer object
    ...
    def find_trainable_layer(self, layer):
        """If a layer is encapsulated by another layer, this function digs
        through the encapsulation and returns the layer that holds the
        weights."""
        if layer.__class__.__name__ == 'TimeDistributed':
...
def mold_image(images, config):
    """Takes RGB images with 0-255 values and subtraces the mean pixel and
...
def trim_zeros_graph(boxes, name=None):
    """Often boxes are represented with matricies of shape [N, 4] and are
    padded with zeros. This removes zero boxes.
